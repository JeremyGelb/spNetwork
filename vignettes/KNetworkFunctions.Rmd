...
As one can see, the theatres seems to be more clustered than the librairies.

```{r fig.align=, fig.show='hold', message=FALSE, warning=FALSE}
...
Note that the cross-K-function A to B is not necessarily the same results as the cross-K-function B to A. Again, in **spNetwork**, the inference is based on Montre Carlo Simulations. The locations of the reference set of points are randomized to estimate if the current situation is more or less clustered that what we could expect at random.
