...
The bandwidth is the most important parameter when doing kernel density estimate. Many methods have been proposed to find an optimal bandwidth but all of them are either crude approximations or require large calculus time. In that context, a theory driven bandwidth is probably the way to go. However, **spNetwork** provides currently two functions to do data driven bandwidth selection:
