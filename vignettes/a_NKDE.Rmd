...
When performing a Network Kernel Density Estimate (NKDE), it is possible to:
...
With this method, one can evaluate the density of the studied phenomenon at each point of the network. In the next figure, 3 sampling points (s1, s2 and s3) are added in blue.
...
with $d_{si}$ the density estimated at sample the point *si*, *n* the number of events within the bandwidth arround the sampling point *si* and *ej* an event.
...
As one can see, most of the kernels look alike. It is worth mentioning that the gaussian kernel does not integrate to 1 within the bandwidth (specific case of non-compact kernel). In the **spNetwork** package, all kernel values beyond the bandwidth are set to 0. This means that some weight of each event is lost with the gaussian kernel. To limit this effect, one can use the scaled version of the gaussian kernel for wich the bandwidth is divided by 3, limiting greatly the mass loss.
...
```
However, the fact that the gaussian kernel does not integrate to 1 within the bandwidth can leads to negative values when using the continuous NKDE (presented latter), we thus recommand using the quartic kernel instead.
...
The **spNetwork** package provides three methods to perform NKDE. We present them briefly here. For more details, please read the original papers and books cited. 
...
$d_{si} = (1/bw) * \sum_{j=1}^{n} K(dist_{si;ej})$
...
Of course, it uses the network distance instead of the euclidean distance. This method is appealing because it is intuitive, but it is not statistically satisfying. Note that the divisor of the kernel is not the classical *1/n*, but *1/bw*. This adjustment allows for a simpler interpretation of the density: *"Instead of calculating the density over an area unit, the equation estimates the density over a linear unit"* [@xie2008kernel p.398]

On the next figure, we show in 3D what that kernel looks like. The red point is a simple event, the black lines are the network, and the blue lines are the evaluated densities.

<center>
![Image Title](images/simple_nkde_ex.png){width=150px}
</center>

As one can see, this NKDE is only determined by the distance between the sampling points and the event. This is a problem because at intersections, the event's mass is multiplied by the number of edges adjacent to that intersection. As a consequence this is not a real kernel because it does not integrate to 1 on its domain.
...
The density of the kernel function is equally divided at intersections. Note that *spNetwork* implements the modified version proposed by @sugihara2010simple, allowing for use in networks with cycles. In the case of a cycle, the multiple values of the kernel that overlap are added, as shown in the next figure.
...
As one can see, the values of the NKDE are split at intersections to avoid the multiplication of the mass observed in the simple version. However, this creates a discontinuous NKDE, which is counter-intuitive. This leads to sharp differences between density values in the network, and could be problematic in networks with a lot of intersections.
...
We can then map the density values estimated for each lixel centre:
...
Let us split the work between two cores.
```{r message=FALSE, warning=FALSE}
...
The examples provided in the previous sections are using only a fixed bandwidth. If we relaxe the need for a fixed bandwidth, we obtain an adaptive estimator. In other words, the bandwidth will vary in the study area. **spNetwork** use the method proposed by @abramson1982bandwidth, implying a variation of the bandwidth that is inversely proportional to the square root of the target density itself. In other words, at places where the density of the spatial process is high, the bandwidth of the kernel will be smaller and inversely. The use of an adaptive bandwidth has many theoritical and pratical advantages. First, it reduces the sensibility to outliers. Second, it reduces smoothing in sub-region with many events (giving more detailed density estimation) and increases the smoothing in sub-region with fiew events (giving more fuzzy results because of the higher uncertainity).

The problem is that the target density itself is unknown (otherwise we would not have to estimate it with a smoothing method). To obtain the adaptive bandwidth, a three steps method is used : 
...
2. For each event, the adaptive bandwidth is calculated with the following formula. A triming values can be defined to limit the maximum adaptive bandwidth.
...
In **spNetwork**, when an adaptive bandwidth is used, the calculated bandwidth for each event are returned too.
...
We could now map some of the bandwidths
```{r message=FALSE, warning=FALSE}
...
```
As one can note, the discontinuous Kernel is maybe not the best option to use with an adaptative bandwidth.

## Applying a correction factor to avoid edge effects

When the study area is limited, the kernel density estimate is biaised at the border because the events at the other side of the border are not sampled. To mitigate the induced biais, one could apply the Diggle correction factor.

$$d(u) = \sum_{i=1}^{n}\frac{1}{e(x_{i})}K(dist(u-x_{i}))\\e(u) = \int_{W}{K(dist(u,v))}
$$
The correction factor $e(u)$ is calculated for each event for which a part of its mass is outside the study area. More precisely, $e(x_{i})$ is the percentage of the mass of the event located inside the study area. The kernel function could be the simple, the continuous or the discontinuous. This correction factor can be used with the adaptive bandiwdth too. Note that this correction is usable only if the network outside the study_area is available.
...
ggplot() + 
  ...
  labs(title = "bike accident density by kilometres in 2016",
          ...
          caption = "using the quartic kernel, adjusted by Diggle factor")
