...
exe_line_ext_listw <- function(i,lines,grid,maxdistance,digits,matrice_type,vdist_func,mindist){
  ...
  if(any(test_lines)==FALSE){
    ...
    #on a maintenant une belle matrice de distance entre les origines et les destinations
    nblist <- lapply(1:nrow(selected_lines),function(j){
      #lets get all the distances from the begining and the end of
      #each line
  ...
  if (show_progress){
    progressr::with_progress({
      ...
      values <- future.apply::future_lapply(iseq,function(i){
        p(sprintf("i=%g", i))
        return(exe_line_ext_listw(i=i,lines=lines,
                         grid=grid,maxdistance=maxdistance,
    ...
    values <-  future.apply::future_lapply(iseq,exe_line_ext_listw,lines=lines,
                     grid=grid,maxdistance=maxdistance,
                     digits=digits,matrice_type=matrice_type,
                     mindist=mindist,
                     vdist_func=vdist_func)
...
line_center_listw_gridded.mc <- function(lines,maxdistance,dist_func="inverse",matrice_type="B",grid_shape=c(2,2), digits=3, show_progress=TRUE){
  ...
  if (show_progress){
    progressr::with_progress({
      ...
      values <- future.apply::future_lapply(iseq,function(i){
        p(sprintf("i=%g", i))
        return(exe_line_center_listw(i=i,lines=lines, centers=centers,
                                  grid=grid,maxdistance=maxdistance,
                                  digits=digits,matrice_type=matrice_type,
                                  vdist_func=vdist_func))
      })
    ...
    values <-  future.apply::future_lapply(iseq,exe_line_center_listw,lines=lines,
                     centers=centers,
                     grid=grid,maxdistance=maxdistance,
                     digits=digits,matrice_type=matrice_type,
                     mindist=mindist,
...
# # listwMP <- line_ext_listw_gridded.mc(mtl_network,maxdistance=1500,
# #                                    dist_func=my_func,
# #                                    matrice_type="W",
# #                                    grid_shape=c(5,5),
# #                                    show_progress=T)
#
# listw <- line_ext_listw_gridded(mtl_network,maxdistance=1500,
#                                         dist_func=my_func,
#                                         matrice_type="W",
#                                         grid_shape=c(5,5),
#                                         show_progress=T)
#
#
# listwMP <- line_center_listw_gridded.mc(mtl_network,maxdistance=1500,
#                                      dist_func=my_func,
#                                      matrice_type="W",
#                                      grid_shape=c(5,5),
#                                      show_progress=T)
...
#
# listwMP2 <- line_center_listw_gridded.mc(mtl_network,maxdistance=1500,
#                                         dist_func=my_func,
#                                         matrice_type="W",
#                                         grid_shape=c(5,5),
#                                         digits = 3,
