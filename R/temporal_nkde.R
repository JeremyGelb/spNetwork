...
#'  are invalid)
#' @param events_loc A SpatialPointsDataFrame representing the aggergated events on the
#' network. The points will be snapped on the network.
#' @param events A SpatialPointsDataFrame representing the base events on the
#' network
...
tnkde_worker <- function(lines, events_loc, events, samples_loc, samples_time, kernel_name,bw_net, bw_time, bws_net, bws_time, method, div, digits, tol, sparse, max_depth, verbose = FALSE){
  ...
  XY_samples <- sp::coordinates(samples_loc)
...
tnkde <- function(lines, events, time_field, w, samples_loc, samples_time, kernel_name, bw_net, bw_time, adaptive=FALSE, trim_bw_net=NULL, trim_bw_time=NULL, method, div="bw", diggle_correction = FALSE, study_area = NULL, max_depth = 15, digits=5, tol=0.1, agg=NULL, sparse=TRUE, grid_shape=c(1,1), verbose=TRUE, check=TRUE){
  ...
  idx <- FNN::knnx.index(coordinates(events_loc),coordinates(events), k = 1)
