...
select_dist_function <- function(dist_func="inverse"){
  if (class(dist_func)=="character"){
    if (dist_func=="identity"){
      dist_func <- function(x){return(x)}
    } else if (dist_func=="inverse"){
      dist_func <- function(x){return(1/x)}
    } else if (dist_func=="squared inverse"){
      dist_func <- function(x){return(1/x**2)}
    }
  }
...
line_ext_listw_gridded <- function(lines,maxdistance,dist_func="inverse",matrice_type="B",grid_shape=c(2,2), show_progress=TRUE, mindist=10, digits=3){
  ...
  if(matrice_type %in% c("B","W","C","U","minmax","S") ==F){
    stop("Matrice type must be B, W, C, U, minmax, or S. See the documentation of spdep::nb2listw")
  }
  ...
  values <- lapply(1:length(grid),function(i){
    ...
    if(any(test_lines)==FALSE){
      ...
      raster::ext <- raster::extent(selected_lines)
      ...
      alldistances1 <- igraph::distances(graph,v=origin_vertices1,to=dest_vertices,mode = "out")
      alldistances2 <- igraph::distances(graph,v=origin_vertices2,to=dest_vertices,mode = "out")
      ...
      if (show_progress){
        ...
        pb <- txtProgressBar(initial=NA,min = 0, max = nrow(selected_lines), style = 3)
        close(pb)
      }
      ...
      nblist <- lapply(1:nrow(selected_lines),function(j){
        ...
        dfdistances <- subset(dfdistances,dfdistances$distance<maxdistance & is.na(dfdistances$distance)==FALSE)
        ...
        if (nrow(combined)>0){
          if (matrice_type=="B"){
            ...
            combined$distance <- ifelse(combined$distance==0,mindist,combined$distance)
            combined$weights <- vdist_func(combined$distance)
    ...
    nbs <- lapply(1:length(nblist),function(j){nblist[[j]][[1]]})
    names(nbs) <- selected_lines$tmpid
    weights <- lapply(1:length(nblist),function(j){nblist[[j]][[2]]})
  ...
  nblist <- do.call("c",lapply(okvalues,function(value){return (value[[1]])}))
  ordered_nblist <- lapply(as.character(lines$tmpid),function(x){return(nblist[[x]])})
  #nblist <- nblist[order(names(nblist))]
  nbweights <- do.call("c",lapply(okvalues,function(value){return (value[[2]])}))
  names(nbweights) <- names(nblist)
  ordered_weights <- lapply(as.character(lines$tmpid),function(x){return(nbweights[[x]])})
  ...
  listw <- spdep::nb2listw(ordered_nblist,glist=ordered_weights,zero.policy = T,style = matrice_type)
  return(listw)
}
...
#'         dist_func = "squared inverse", matrice_type="B",
#'         grid_shape = c(5,5))
line_center_listw_gridded <- function(lines,maxdistance,dist_func="inverse",matrice_type="B",grid_shape=c(2,2), show_progress=TRUE, digits=3){
  ##checking the matrix type
  ...
  #lines <- add_vertices_lines(lines,centers)
  ...
  values <- lapply(1:length(grid),function(i){
    ...
    if(any(test_lines)==FALSE){
      ...
      vertices <- find_vertices(spvertices = result_graph$spvertices, points = selected_centers, digits = digits)
      selected_centers$vertex <- vertices
      #u <- igraph::V(graph)
      u <- find_vertices(spvertices = result_graph$spvertices, points = graph_centers, digits = digits)
      #calculating the distances
...
#
#
# find_mat_error <- function(Mat){
#   dimensions <- dim(Mat)
#   errors <- list()
#   for (i in 1:dimensions[[1]]){
#     for(j in 1:dimensions[[2]]){
#       if(abs(Mat[i,j]-Mat[j,i])>0.01){
#         errors[[length(errors)+1]]<-c(i,j)
#       }
#     }
#   }
...
# }
#
# listw <- line_center_listw_gridded(mtl_network,maxdistance=1500,
#                             dist_func=my_func,
#                             matrice_type="W",
#                             grid_shape=c(5,5),
#                             show_progress=TRUE)
# is.symmetric.nb(listw$neighbours)
#
# listw2 <- line_ext_listw_gridded(mtl_network,maxdistance=1500,
#                                    dist_func=my_func,
#                                    matrice_type="W",
#                                    grid_shape=c(5,5),
#                                    show_progress=TRUE)
# is.symmetric.nb(listw2$neighbours)
#
# future::plan(future::multiprocess(workers=4))
# listw3 <- line_center_listw_gridded.mc(mtl_network,maxdistance=1500,
#                                    dist_func=my_func,
#                                    matrice_type="W",
#                                    grid_shape=c(5,5),
#                                    show_progress=TRUE)
# is.symmetric.nb(listw3$neighbours)
#
# listw4 <- line_ext_listw_gridded.mc(mtl_network,maxdistance=1500,
#                                        dist_func=my_func,
#                                        matrice_type="W",
#                                        grid_shape=c(5,5),
#                                        show_progress=TRUE)
# is.symmetric.nb(listw4$neighbours)
#
#
...
# }
#
# CompareNeighbours(listw$neighbours,listw3$neighbours)
# CompareWeights(listw$weights,listw3$weights)
# CompareNeighbours(listw2$neighbours,listw4$neighbours)
# CompareWeights(listw2$weights,listw4$weights)
#
# # listw <- line_listw_gridded(mtl_network,maxdistance=1500,
# #                             dist_func=my_func,
# #                             matrice_type="W",
# #                             grid_shape=c(5,5),
# #                             show_progress=TRUE)
...
# #
# # new_pts <- snapPointsToLines(bike_accidents,lines = mtl_network,maxDist = 150,withAttrs = T)
# #
# # writeOGR(new_pts,"data/new_pts.gpkg",layer="acc",driver="GPKG")
# #
# # nbpt <- sapply(1:nrow(mtl_network),function(i){
# #   line <- mtl_network[i,]
# #   buff <- gBuffer(line,width=0.1)
# #   test <- gIntersects(new_pts,buff,byid = T)
# #   tot <- sum(test, na.rm = TRUE)
# #   return(tot)
# # })
# #
# # mtl_network$nbAccident <- nbpt
# #
#
# #
# #
# # MoranI <- localmoran(mtl_network$nbAccident,listw,zero.policy = T)
# #
# # Final <- cbind(mtl_network,MoranI)
# # writeOGR(Final,"moranlines.gpkg",driver="GPKG",layer="moran")
# #
# # # LyonIris <- readOGR("tempLyon.gpkg")
# # # queen <- poly2nb(LyonIris)
# # #
# # # alllengths <- lapply(queen,function(e){return(length(e))})
# # #
# # #
# # # listw <- nb2listw(queen,zero.policy = T)
# # #
# # # writeOGR(LyonIris,"tempLyon.gpkg",layer="lyon",driver='GPKG')
